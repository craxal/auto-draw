...
export type RuntimeError = { token: Token; message: string; };

function tryNativeFunction(operation: () => any, token: Token): RuntimeResult {
    try {
...
export class Interpreter2 implements IProgramVisitor<RuntimeResult> {
    ...
    #console: string[] = [];
        ...
        ['ArcLeft', { arity: 2, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.arcLeft(args[0], args[1]), token); } }],
        ['ArcRight', { arity: 2, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.arcRight(args[0], args[1]), token); } }],
        ['MoveForward', { arity: 1, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.moveForward(args[0]), token); } }],
        ['PenColor', { arity: 1, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.setPenColor(args[0]), token); } }],
        ['PenDown', { arity: 0, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.penDown(), token); } }],
        ['PenUp', { arity: 0, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.penUp(), token); } }],
        ['TurnLeft', { arity: 1, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.turnLeft(args[0]), token); } }],
        ['TurnRight', { arity: 1, call(interpreter: Interpreter2, token: Token, args: any[]) { return tryNativeFunction(() => interpreter.#context?.turnRight(args[0]), token); } }],
        ['Print', { arity: 1, call(interpreter: Interpreter2, token: Token, args: any[]) { console.log(`DEBUG >>> Printing '${args[0]}'`); return tryNativeFunction(() => interpreter.#console.push(args[0].toString()), token); } }],
    );
    ...
    #lookupVariable(name: Token, expression: Expression): RuntimeResult {
        const distance = this.#locals.get(expression);
        if (distance !== undefined) {
    ...
    #checkOperands(operator: Token, ...operands: any[]): RuntimeError | undefined {
